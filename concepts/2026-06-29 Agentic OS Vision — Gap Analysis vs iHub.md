...
> **Re‑verified 2026‑08‑26** against `main` @ `db85096` (tip: *RAGFlow comparison* #2225). The tree advanced substantially since the original pass (the branch's history was rebuilt, so this is a fresh re‑check of every claim, not a diff). **Net: the conclusions hold — no P0/P1 gap has been closed.** Deltas found this pass:
>
> - **Economics is now _partly_ enforced.** Agent profiles carry a **per‑run token budget** — `agentProfileSchema.budgets.maxTokensPerRun` — that halts the tool loop once run‑level token spend crosses a ceiling (Claude‑Code‑style gating; the running total lives on the workflow state, `PromptNodeExecutor.js`). This is in addition to the wall‑time timeout and the planner's plan‑task budget. Still missing: **monetary** spend‑caps / auto‑pause, cost‑aware model routing (cheap↔strong), and per‑request/model/app cost attribution.
> - **Execution fabric, sharpened (still ❌).** The server forks a round‑robin **cluster** (default 4 workers, `STICKY_SESSIONS=false`); cross‑worker in‑process state is a *known hazard* patched case‑by‑case over an IPC `clusterBus` (see *2026‑07‑28 Round‑Robin Worker‑Local State Audit*). That is stateless‑HTTP scaling, **not** the prompt's durable task‑claiming worker fabric — no job queue, no pull‑based claiming, no multi‑machine.
> - **Narrow quarantine exists.** A run with no `triggeredBy` is now quarantined (`routes/agents/runs.js`), but there is still no general dead‑letter / requeue queue.
> - **Sizes drifted:** `WorkflowEngine.js` ~1.6k→**1.8k** LOC, `PlannerNodeExecutor.js` ~1.2k→**1.5k** LOC; node‑executor count unchanged (**24**). Confirmed still absent: `evals/` harness, autonomy/trust levels, semantic/vector memory, momentum queues, `project` entity, universal ask bar.
...
| **P0** | **Economics**: cost‑aware model routing, **monetary** budgets/auto‑pause, spend caps | Partial — per‑run *token* gating now exists (`maxTokensPerRun`); **monetary** caps + cost‑aware routing still absent |
...
| **Decompose → task graph** | `PlannerNodeExecutor` (LLM decomposition) → `_taskQueue` + materialized sub‑DAG | `executors/PlannerNodeExecutor.js` (1473 LOC); `SubWorkflowMaterializer.js` |
| **Route + execute** | `WorkflowEngine` + `DAGScheduler` (Kahn, AND/OR deps, fan‑out/in), 24 node executors, multi‑step tool loop | `WorkflowEngine.js` (1816 LOC); `DAGScheduler.js`; `services/chat/ToolExecutor.js` |
...
| **B. Execution fabric (pull‑based workers, hub‑and‑worker, multi‑machine)** | ❌ | **Round‑robin Node cluster** (default 4 workers) but **stateless‑HTTP, not a task fabric**: workflow/agent runs execute in‑process on whichever worker holds the request; cross‑worker in‑process state is a known hazard patched case‑by‑case over an IPC `clusterBus` (*2026‑07‑28 Worker‑Local State Audit*). No job queue, no pull‑based claiming, no worker daemons, no multi‑machine. Branch‑level parallelism only (`ParallelNodeExecutor`, `Promise.allSettled`). |
...
| **G. Model routing + economics** | 🟡 | 8 providers w/ normalized tool‑calling = ✅. Per‑run **token‑budget gating = ✅** (`agentProfileSchema.budgets.maxTokensPerRun` halts the tool loop at a token ceiling; spend tracked on workflow state). **Cost‑aware routing (cheap↔strong per task) = ❌. Monetary budgets / spend‑caps / auto‑pause = ❌** (model cost fields are telemetry‑only). `server/adapters/`, `services/UsageEventLog.js` |
...
| **Dead‑letter / quarantine queue** | ❌ | Failed runs persist as `failed`. One narrow quarantine case (a run with no `triggeredBy`, `routes/agents/runs.js`); no general requeue / dead‑letter queue |
...
5. **Monetary cost budgets with auto‑pause** at task/goal/profile/user/month layers. (Today a per‑run *token* budget is enforced — `budgets.maxTokensPerRun` — alongside the agent wall‑time timeout; neither maps spend to money nor caps it across users/month.)
