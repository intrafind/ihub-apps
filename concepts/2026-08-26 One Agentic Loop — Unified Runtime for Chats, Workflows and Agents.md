...
### 10.3 Questions we had not asked — the decision list before C0

1. **Is the chat SSE/API a documented external contract?** Internal-only → C4 clean break as planned. Customer-consumed → the break needs versioned comms and possibly a support window. Owner: product.
2. **Who owns conversation state — client or server?** Phase 1 answer proposed here: client-authoritative conversation, server-side ledger as audit + resume substrate for pauses; server-authoritative sessions (client sends only the new message + runId) become a separate, later decision with its own API break. Needs explicit sign-off because edit/regenerate semantics hang on it.
3. **Ledger privacy and erasure.** Storing employee chat transcripts server-side is new personal data: per-app opt-in, admin disclosure, retention per kind — and **user-requested conversation deletion must delete/redact the run log** (append-only ≠ undeletable). For German enterprise deployments this likely needs works-council/DSGVO review before default-on. Owner: product + legal.
4. **Anonymous mode.** How is answering/resuming a paused run authorized for anonymous users (session-cookie binding?), and are anonymous transcripts stored at all (proposal: interactions and headers yes, message bodies no, unless the deployment opts in)? Today's `anonymousAuth` installs must not silently start persisting chats.
5. **Per-kind ledger policy for high-QPS surfaces.** Does `kind: 'inference'` log full request headers or hash-only with sampled full captures? Cost/volume decision, not architecture.
6. **Process discipline during the cutover.** Once C0 lands: a "no new features on legacy loops" freeze (otherwise C1–C3 fight merge conflicts forever), stage-sized PRs, and who staffs/owns the effort and the release train for the C4 major version.
