...
The project uses a versioned migration system (similar to Flyway) for configuration changes. Migrations live in `server/migrations/` and run automatically on server startup before `configCache` initializes.

#### When to Write a Migration

Create a new migration whenever you:
- Add new required or default fields to existing config files (e.g., new section in `platform.json`)
...
- Remove or move configuration keys that existing installations may still have in old format
...
1. Find the next available version number by checking `server/migrations/V*.js` files.
...
- **Never modify a migration file after it has been applied** — checksums are tracked; mismatches trigger warnings or halt startup.
- Migrations are **forward-only**. To undo a change, create a new higher-versioned migration.
- History is stored at `contents/.migration-history.json`. Each migration records status, checksum, and timing.
