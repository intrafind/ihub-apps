...
| `LLM_CONNECT_TIMEOUT_MS`   | Longest a **streamed** provider call waits for the response headers before failing as unreachable, per attempt. Queue time in the per-model throttle does not count, and non-streamed calls are governed by `REQUEST_TIMEOUT` instead (their headers arrive with the finished answer). `0` disables it. Overridable per installation via `llm.connectTimeoutMs` in `platform.json` and per model via `connectTimeoutMs`. See [Stream deadlines](llm-client.md#stream-deadlines). | `10000` |
