...
iHub Apps supports automatic watermarking and metadata tagging for AI-generated images. This feature adds installation-specific (and optionally user-specific) watermarks to all images generated by image generation models like Gemini 2.5 Flash Image.

### Configuration

Watermarking is configured in the `platform.json` file:
...
|----------|------|---------|-------------|
| `enabled` | Boolean | `false` | Enable or disable watermarking |
| `text` | String | `"iHub Apps"` | Watermark text (installation-specific) |
| `position` | String | `"bottom-right"` | Watermark position: `top-left`, `top-right`, `bottom-left`, `bottom-right`, `center` |
| `opacity` | Number | `0.5` | Watermark opacity (0.0 to 1.0) |
| `textColor` | String | `"#ffffff"` | Text color in hex format |
| `includeUser` | Boolean | `false` | Include username in watermark |
| `includeTimestamp` | Boolean | `false` | Include date in watermark |
| `installationId` | String | `""` | Optional installation identifier for metadata |

### Features

**Visual Watermarking**:
- Configurable text overlay with shadow for readability
- Multiple position options (corners and center)
- Adjustable opacity and color
- Automatic font scaling based on image dimensions

**User-Specific Watermarks**:
When `includeUser` is enabled, the watermark includes the username:
```
"iHub Apps | john.doe"
```

When both `includeUser` and `includeTimestamp` are enabled:
```
"iHub Apps | john.doe | 2026-02-20"
```

**EXIF/IPTC Metadata**:
The following metadata is automatically embedded in generated images:
- **Artist**: Username or user ID (if `includeUser` is enabled)
- **Copyright**: Copyright notice with current year
- **DateTimeOriginal**: ISO timestamp of generation
- **Software**: Installation name from watermark text
- **ImageDescription**: Generator and installation information

### Usage

1. **Enable watermarking** in `contents/config/platform.json`
2. **Configure settings** according to your needs
3. **Restart server** to apply changes
4. **Generate images** - watermarks are applied automatically

No client-side configuration needed. All image generation models automatically apply watermarks when enabled.
...
Watermarking adds minimal overhead to image generation:
- Processing time: < 100ms per image
- Uses efficient sharp library (libvips-based)
- Non-blocking async processing
- No impact on streaming or chat functionality

### Troubleshooting

**Watermark not appearing**:
- Check `enabled` is set to `true`
- Verify server restart after configuration change
- Check server logs for watermarking errors

**Metadata not embedded**:
- Ensure `sharp` library is installed: `npm install` in server directory
- Verify image format is PNG or JPEG
- Check EXIF viewer supports the metadata fields

### Future Enhancements

Planned improvements include:
- C2PA (Content Provenance and Authenticity) support
- Logo watermarks (images instead of text)
