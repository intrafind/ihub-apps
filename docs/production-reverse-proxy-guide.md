...
http {
    ...
    server {
        ...
        # WebSocket for realtime voice (dictation + transcription).
        # See docs/voice-transcription.md for the full guide. The backend pings
        # every 25s, so these timeouts are belt-and-braces.
        location /ihub/api/voice/realtime {
            ...
            # X-Forwarded-Host is REQUIRED for the backend's same-origin
            # (CSWSH) check when Host is rewritten to an internal name.
            proxy_set_header Host $host;
