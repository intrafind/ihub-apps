...
empty under **Admin → Voice Input** and set the host to the container: the browser now connects
to it directly. Azure cloud still needs a key. A failed Azure setup now shows its error instead
