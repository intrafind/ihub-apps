  ...
  describe('Cross-Provider Consistency Tests', () => {
    test('should produce consistent response structure across providers', async () => {
      ...
      for (const provider of providers) {
        ...
        if (testEnvironment.enableRealApiCalls && process.env[`${provider.toUpperCase()}_API_KEY`]) {
          try {
            const request = createCompletionRequest(model, [{ role: 'user', content: testMessage }], apiKey, {
              temperature: 0.1,
              maxTokens: 20,
              stream: false
            });
