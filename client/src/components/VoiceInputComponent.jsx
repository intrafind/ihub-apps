...
const VoiceInputComponent = ({ onSpeechResult, inputRef, disabled = false, onCommand = null }) => {
  ...
  useEffect(() => {
    ...
    if (!document.getElementById('voice-feedback')) {
      ...
      instructions.textContent = t('voiceInput.instructions', 'Speak clearly and naturally');
    ...
    for (const [command, patterns] of Object.entries(commandPatterns)) {
      for (const pattern of patterns) {
        ...
        if (lowerText.endsWith(pattern)) {
          console.log(`Command detected: "${command}" with pattern: "${pattern}"`);
  ...
  const startListening = async () => {
    ...
    try {
      ...
      if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
        showError(t('voiceInput.error.notSupported', 'Speech recognition not supported in this browser'));
        return;
      }
      ...
      try {
        ...
        showError(t('voiceInput.error.permissionDenied', 'Please allow microphone access and try again.'));
        return;
      }
      ...
      recognition.onresult = (event) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          console.log(`Transcript ${i}:`, transcript, isFinal ? '(final)' : '(interim)');
          ...
          if (command && onCommand) {
            ...
            if (cleanText) {
              ...
              if (inputRef?.current) {
                ...
                // Update React state
                if (onSpeechResult) {
                  console.log('Calling onSpeechResult with clean text:', newValue);
      ...
      recognition.onerror = (event) => {
        ...
        switch (event.error) {
          case 'no-speech':
            ...
            break;
          case 'audio-capture':
            errorMsg = t('voiceInput.error.noMicrophone', 'No microphone found. Please check your device settings.');
            break;
          case 'not-allowed':
            errorMsg = t('voiceInput.error.permissionDenied', 'Please allow microphone access and try again.');
            break;
          case 'network':
            errorMsg = t('voiceInput.error.network', 'Network error. Please check your connection.');
            break;
          default:
      ...
      recognition.onend = () => {
        ...
        if (inputRef?.current) {
          inputRef.current.placeholder = t('voiceInput.messagePlaceholder', 'Type your message here...');
        }
