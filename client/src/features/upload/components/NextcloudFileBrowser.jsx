...
/**
 * Nextcloud File Browser component.
 *
 * Same drive → folder → file flow as the Office 365 / Google Drive
 * browsers — the underlying API is WebDAV (path-based) rather than
 * Microsoft Graph (id-based), but the UI shape is identical so the
 * three providers feel uniform to the end user.
 */
const NextcloudFileBrowser = ({ provider, onFilesProcessed, onClose, uploadConfig }) => {
  ...
  const handleConnect = () => {
    ...
    const authUrl = `/api/integrations/nextcloud/auth?providerId=${encodeURIComponent(provider.id)}&returnUrl=${encodeURIComponent(returnUrl)}`;
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
          {t('cloudStorage.nextcloud.notConnected', 'Nextcloud Not Connected')}
        </h3>
        ...
        <div className="flex gap-3">
          ...
          >
            {t('cloudStorage.nextcloud.connect', 'Connect to Nextcloud')}
          </button>
        </div>
      </div>
    );
