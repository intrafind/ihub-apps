...
const GoogleDriveFileBrowser = ({ provider, onFilesProcessed, onClose, uploadConfig }) => {
  ...
  const processedItems = useMemo(() => {
    ...
    if (!showUnsupportedFiles) {
      result = result.filter(
        item => item.isFolder || item.isGoogleDoc || isCloudFileSupported(item.mimeType)
      );
    }
  ...
  const canSelectFile = item => {
    ...
    // Google Docs can always be selected (size is 0 but they export to standard formats)
    if (item.isGoogleDoc) return isCloudFileSupported(item.mimeType);
  ...
  const getFileTooltip = item => {
    ...
    if (!supported && !item.isGoogleDoc)
      return t('upload.fileTypeNotSupported', 'File type not supported');
    if (!item.isGoogleDoc && item.size > maxSize)
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
          {t('cloudStorage.googleDrive.notConnected', 'Google Drive Not Connected')}
        </h3>
        ...
        <div className="flex gap-3">
          ...
          >
            {t('cloudStorage.googleDrive.connect', 'Connect to Google Drive')}
          </button>
        </div>
      </div>
    );
                    ...
                    >
                      ...
                      <div className="flex items-center ml-4">
                        <div className="w-32 text-right text-xs text-gray-500 dark:text-gray-400">
                          ...
                          {!isFolder && item.isGoogleDoc && item.size === 0 && (
                            <span className="text-gray-400 italic">Google Doc</span>
                          )}
                        </div>
