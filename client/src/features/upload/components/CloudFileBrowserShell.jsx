 ...
 * (Office 365 / Google Drive / Nextcloud).
 *
 * Each provider component is a ~15-line wrapper that supplies the
 * provider-specific hook (built with `useCloudStorageBrowser`), the i18n
 * key set for the connect-prompt view, and a couple of optional adapter
 * predicates to handle provider-specific oddities like Google's virtual
 * "Google Docs" entries.
 */
const CloudFileBrowserShell = ({
  provider,
  useBrowserHook,
  i18nKeys,
  uploadConfig,
  onFilesProcessed,
  onClose,
  // Optional adapter: predicate marking provider-specific virtual files
  // that have no upstream size but are still attachable (Google Docs).
  isVirtualFile = () => false,
  // Optional adapter: extra MIME → icon rules for provider-specific
  // document types (e.g. Google Sheets / Slides).
  extraFileIconRule = () => null
}) => {
  ...
  const processedItems = useMemo(() => {
    ...
    if (!showUnsupportedFiles) {
      result = result.filter(
        item => item.isFolder || isVirtualFile(item) || isCloudFileSupported(item.mimeType)
      );
    }
    ...
    return result;
  }, [items, showUnsupportedFiles, isVirtualFile, sortBy, sortDirection]);
  ...
  const getFileIcon = item => {
    ...
    const extra = extraFileIconRule(item);
  ...
  const canSelectFile = item => {
    ...
    if (isVirtualFile(item)) return supported;
  ...
  const getFileTooltip = item => {
    ...
    const maxSize = (uploadConfig.maxFileSizeMB || 10) * 1024 * 1024;
    const supported = isCloudFileSupported(item.mimeType);
    const virtual = isVirtualFile(item);

    if (!supported && !virtual) {
      return t('upload.fileTypeNotSupported', 'File type not supported');
    }
    if (!virtual && item.size > maxSize) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
        ...
        <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
          {t(i18nKeys.notConnected[0], i18nKeys.notConnected[1])}
        </h3>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 max-w-sm">
          {t(i18nKeys.connectPrompt[0], i18nKeys.connectPrompt[1])}
        </p>
  ...
  // View C: Drive Selection (skipped when source has exactly one drive)
  ...
  // View D: File Browser
  if (currentDrive) {
    // When the drive list was auto-skipped (single-drive source like
    // `myDrive` or Nextcloud), "Back" jumps straight to source list.
    const backLabel = autoSkippedDrives
      ? t('cloudStorage.backToStorageLocations', 'Back to Storage Locations')
      : t('cloudStorage.backToDrives', 'Back to Drives');
    const onBack = autoSkippedDrives ? goBackToSources : () => selectDrive(null);
        ...
        <div className="mb-4 pb-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 flex-wrap">
            {breadcrumbs.map((crumb, index) => (
              <div key={crumb.path ?? crumb.id ?? `${index}`} className="flex items-center">
                    ...
                    >
                      ...
                      <div className="flex items-center ml-4">
                        <div className="w-32 text-right text-xs text-gray-500 dark:text-gray-400">
                          {!isFolder && item.size > 0 && <span>{formatFileSize(item.size)}</span>}
                          {!isFolder && virtual && item.size === 0 && (
                            <span className="text-gray-400 italic">
                              {t('cloudStorage.virtualFile', 'Cloud document')}
                            </span>
                          )}
                        </div>
...
export default CloudFileBrowserShell;
