...
const OfflineIndicator = () => {
  ...
  if (connectionState === CONNECTION_STATES.CHECKING) {
    return null;
  }
    ...
    switch (connectionState) {
      ...
      case CONNECTION_STATES.BACKEND_OFFLINE:
        return {
          ...
          message: t('network.status.backend_offline.message', 'Unable to reach the server'),
          showRetry: false
        };
    ...
    >
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        <div className="flex items-center space-x-3">
          <Icon
            name={config.icon}
            className={`w-5 h-5 ${connectionState === CONNECTION_STATES.CHECKING ? 'animate-spin' : ''}`}
          />
