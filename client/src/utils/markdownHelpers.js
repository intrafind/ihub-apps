...
export const validateMermaidCode = (code) => {
  ...
  const hasValidStart = /^(graph|flowchart|sequenceDiagram|classDiagram|stateDiagram|erDiagram|journey|gantt|pie|timeline|mindmap|gitgraph|quadrantChart)/i.test(lines[0]) ||
...
export const processMermaidCode = (code) => {
  ...
  if (code.includes('<br>') || code.includes('<br/>') || code.includes('<br />')) {
    ...
    processedCode = processedCode.replace(/(\w+)(\[)([^\]]+)(\])/g, (match, nodeId, openBracket, content) => {
...
export const detectDiagramType = (code) => {
  ...
  if (lines.length > 1 && lines.slice(1).some(line => line.trim() && !line.trim().includes('-->') && !line.trim().includes('---'))) {
    // If content has indentation-based structure without arrows, likely mindmap
    const hasIndentedContent = lines.slice(1).some(line => line.startsWith('    ') || line.startsWith('\t'));
