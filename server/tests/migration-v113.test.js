...
/**
 * Migration V113 specs — grandfathering.
 *
 * V112 turns the approval gate on. This migration is the only reason that is
...
import {
  ...
  version,
  description
} from '../migrations/V113__grandfather_connected_cimd_clients.js';
...
describe('V113 — grandfather connected CIMD clients', () => {
  before(async () => {
    baseDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ihub-v113-'));
  });
  after(async () => {
