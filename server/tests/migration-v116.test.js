...
/**
 * Migration V116 specs — the Staan provider reaches existing installs.
 *
...
import {
  ...
  version,
  description
} from '../migrations/V116__add_staan_websearch_provider.js';
...
describe('V116 — Staan web search provider', () => {
  before(async () => {
    baseDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ihub-v116-'));
  });
  after(async () => {
