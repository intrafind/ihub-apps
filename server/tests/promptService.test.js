...
// and a client-supplied variable key with regex metacharacters must not throw
async function testSystemPromptWithDollarPatternAndMetacharacterKey() {
  const messages = [
    {
      role: 'user',
      content: 'Hi',
      variables: { location: '$100 & change', 'weird(key)': 'still works' }
    }
  ];

  const app = {
    system: { en: 'Location: {{location}} / Weird: {{weird(key)}}' }
  };
  ...
  assert.strictEqual(
    ...
    'Location: $100 & change / Weird: still works',
    'System prompt should substitute $-pattern values literally and not throw on metacharacter keys'
  );

  logger.info('✓ System prompt with $-pattern value and metacharacter key test passed');
}
...
async function runTests() {
  try {
    ...
    await testSystemPromptWithDollarPatternAndMetacharacterKey();
