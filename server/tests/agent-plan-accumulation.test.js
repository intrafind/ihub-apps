...
async function run() {
  ...
  check('round 0: empty prior → just the new tasks', m0.length === 2 && m0[0].id === 'bio-career-research');
  ...
  check('round 1: round-0 tasks come first (order preserved)', m1[0].id === 'bio-career-research' && m1[1].id === 'ai-expertise-research');
  check('round 1: round-1 tasks appended after', m1[2].id === 'r1_verify-dates' && m1[3].id === 'r1_final-report-revision');
  ...
  check('idempotent: same prior+incoming → same length, no dupes', again.length === 4 && reMerged.length === 4);
  ...
  check('same id: incoming metadata wins', refreshed[0].title === 'new' && refreshed[0].status === 'done');

  // Defensive: non-array inputs never throw.
  check('non-array inputs are safe', Array.isArray(p._mergePlanTasks(null, null)) && p._mergePlanTasks(null, null).length === 0);
