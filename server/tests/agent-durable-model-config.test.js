...
function run() {
  ...
  check('falls to run-wide default when no per-node override',
  ...
  check('no nodeId → run-wide default', v.resolveConfiguredModelId(state) === 'gemini-flash-latest');

  // --- resolveModel honors the durable config over the global default ---
  const mVerify = v.resolveModel(models, {}, {}, state, 'verify');
  check('verify resolves durable per-node model (NOT local-vllm)', mVerify?.id === 'gemini-3.1-pro', mVerify?.id);

  const mPlanner = v.resolveModel(models, {}, {}, state, 'planner');
  check('node w/o override resolves durable default (NOT local-vllm)', mPlanner?.id === 'gemini-flash-latest', mPlanner?.id);
  ...
  const mWf = v.resolveModel(models, {}, { workflow: { config: { defaultModelId: 'gemini-flash-latest' } } }, state, 'verify');
