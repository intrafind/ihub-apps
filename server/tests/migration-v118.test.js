...
/**
 * Migration V118 specs — `useLocalOfficejs` becomes an Office.js source mode.
 *
...
import {
  ...
  version,
  description
} from '../migrations/V118__office_js_source_modes.js';
...
before(async () => {
  baseDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ihub-v118-'));
});
...
describe('V118 — Office.js source modes', () => {
  it('is registered as version 118', () => {
    assert.equal(version, '118');
