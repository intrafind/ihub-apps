...
/**
 * Migration V129 specs — seeding `platform.chats.sharing`.
 *
...
import {
  ...
  description,
  SHARING_DEFAULTS
} from '../migrations/V129__chat_sharing_defaults.js';
...
async function seed(platform) {
  const dir = await fs.mkdtemp(path.join(baseDir, 'v129-'));
...
before(async () => {
  baseDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ihub-migration-v129-'));
});
...
describe('V129 seeds the sharing block', () => {
