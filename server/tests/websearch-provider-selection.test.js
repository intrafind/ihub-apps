 ...
 * one worth pinning down: it has to mean "Brave if this install has a Brave key,
 * Qwant otherwise", because the whole point of adding a keyless engine is that
 * an install without a search subscription stops offering the model a tool that
 * fails on every call.
 *
...
const withBraveKey = { braveConfigured: () => true };
const withoutBraveKey = { braveConfigured: () => false };
...
describe('resolveWebsearchToolId', () => {
  ...
  it('auto falls back to the keyless Qwant when Brave has no key', () => {
  ...
  it('only ever names a tool that exists', () => {
    ...
    for (const provider of ['auto', 'brave', 'qwant', 'nonsense', undefined]) {
      for (const deps of [withBraveKey, withoutBraveKey]) {
