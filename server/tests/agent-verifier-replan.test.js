...
async function run() {
  ...
  const r = v.interpretResult(
    { verdict: 'FAIL', failures: ['Wrong date for X'] },
    { mode: 'adversarial' }
  );
  ...
  check(
    'carries the gaps for the planner',
    JSON.stringify(upd._lastReviewGaps) === JSON.stringify(['Wrong date for X'])
  );
