...
describe('Group Handling System Tests', () => {
  ...
  describe('OIDC Authentication', () => {
    ...
    test('should merge OIDC external groups with admin-assigned internal groups', async () => {
      ...
      const validatedUser = await validateAndPersistExternalUser(
        existingOidcUser,
        mockPlatformConfig
      );
    ...
    test('should handle OIDC self-signup when enabled', async () => {
      ...
      fs.readFileSync.mockReturnValue(
        JSON.stringify({ users: {}, metadata: { version: '2.0.0' } })
      );
    ...
    test('should reject OIDC self-signup when disabled', async () => {
      ...
      await expect(
        validateAndPersistExternalUser(newOidcUser, restrictivePlatform)
  ...
  describe('Error Handling', () => {
    test('should handle disabled user accounts', async () => {
      ...
      await expect(
        validateAndPersistExternalUser(disabledUser, mockPlatformConfig)
