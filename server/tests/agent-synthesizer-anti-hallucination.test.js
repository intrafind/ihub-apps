...
const QUOTE_RULE =
  /(never|do not|don't).{0,60}(alter|fabricat|invent|paraphrase|change).{0,60}quot|quot.{0,60}(verbatim|exact)/i;
const DATE_RULE =
  /(never|do not|don't).{0,80}(invent|guess|fabricat|extrapolat).{0,40}(date|publication)|future.{0,40}date/i;
...
const SUMMARY_RULE =
  /summary.{0,160}(must not|not).{0,40}(stronger|exceed|overstate|escalat|more confiden)/i;
const ATTRIBUTION_RULE =
  /attribut.{0,160}(never|not).{0,80}(figure|quote|percentage|claim|source)|never attribute/i;
...
function hasGuardrails(rawText, where) {
  ...
  check(
    `${where}: forbids altering/fabricating quotes`,
    QUOTE_RULE.test(text),
    text ? '(present but no match)' : '(empty)'
  );
...
function hasSynthDiscipline(rawText, where) {
  const text = normalize(rawText);
  check(
    `${where}: summary must not overstate the body`,
    SUMMARY_RULE.test(text),
    text ? '(present but no match)' : '(empty)'
  );
