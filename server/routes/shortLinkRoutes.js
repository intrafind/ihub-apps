import { createLink, getLink, recordUsage, deleteLink, updateLink, searchLinks } from '../shortLinkManager.js';

export default function registerShortLinkRoutes(app) {
  app.post('/api/shortlinks', async (req, res) => {
    try {
      const { code, appId, userId, path, params, url, includeParams } = req.body;
      if (!url && !appId && !path) {
        return res.status(400).json({ error: 'appId or url or path required' });
      }
      const link = await createLink({ code, appId, userId, path, params, url, includeParams });
      res.json(link);
    } catch (e) {
