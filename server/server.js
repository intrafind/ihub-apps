  ...
  try {
    ...
    if (teamsAdapter) {
      ...
      app.get('/teams/manifest.json', async (req, res) => {
        try {
          const { loadTeamsConfiguration, loadAppConfigurations } = await import(
            './configCache.js'
          );
          const { enhanceUserWithPermissions, filterResourcesByPermissions } = await import(
          ...
          const allowedApps = filterResourcesByPermissions(
            apps,
    ...
    if (commands.length === 0) {
      const defaultCommands = [
        ...
        },
        {
          ...
          id: 'analyze',
          title: 'Analyze',
          description: 'Analyze the selected content'
        },
        {
          appId: 'email-composer',
          id: 'improve-writing',
