...
export default async function enhancedWebSearch({ 
  ...
  try {
    ...
    if (extractContent) {
      ...
      extractedContent.push(...resultsToProcess.map(result => ({
        ...result,
        contentExtracted: false,
        extractionError: null
      })));
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!query) {
    console.error('Usage: node enhancedWebSearch.js <search term> [--no-extract] [--max-results=N]');
