/**
 * Migration V128 — iFinder: document ids for follow-up calls
 *
 ...
 *
 * V127 (date-only prompts) rewrote one sentence of the same app prompt, but only
 * where the result is exactly the shipped text — which it no longer is once the
 * answer rules changed too. So this migration accepts both the pre-V127 text and
 * the text V127 produces from it, and lands both on the current default.
 */
...
export function applyAppPrompt(app, shipped) {
  ...
  const refreshed = [];
  for (const [locale, supersededTexts] of Object.entries(SUPERSEDED_APP_PROMPT)) {
    const current = shippedSystem[locale];
    if (typeof current !== 'string' || system[locale] === current) continue;
    if (!supersededTexts.includes(system[locale])) continue;
